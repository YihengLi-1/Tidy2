...
final class Indexer: IndexerServiceProtocol {
    ...
    func scanDownloads(rootURL: URL) throws -> [IndexedFile] {
        try runDownloadsScan(rootURL: rootURL, forceFullScan: false, reason: "scanDownloads")
    }

    func forceFullScanDownloads(rootURL: URL) throws -> [IndexedFile] {
        try store.setDoubleSetting(key: watermarkKey, value: 0)
        return try runDownloadsScan(rootURL: rootURL, forceFullScan: true, reason: "forceFullScan")
    }
    ...
    private func runDownloadsScan(rootURL: URL, forceFullScan: Bool, reason: String) throws -> [IndexedFile] {
