...
actor FileIntelligenceService {
    ...
    - keepOrDelete: keep|delete|unsure
      - delete: 明确是安装包(.dmg/.pkg)、明显的临时文件、可执行文件
      - unsure: 仅当文件类型极其模糊且无任何可识别内容时
      - keep: 其他所有情况（截图也算keep，建议归档到截图文件夹）
    - reason: 分类依据，≤60字
    - confidence: 0.0-1.0
    """
    ...
    /// Returns true for source-code and developer files that should never be user documents.
    private static let developerExtensions: Set<String> = [
        "swift", "m", "h", "mm", "cpp", "c", "cs", "java", "py", "rb",
        "go", "rs", "kt", "ts", "js", "jsx", "tsx", "vue", "html", "css",
        "scss", "sass", "less", "sh", "bash", "zsh", "fish", "ps1",
        "xcodeproj", "xcworkspace", "pbxproj", "gradle", "pom",
        "lock", "log", "map"
    ]

    /// Returns true for binary/data files with no meaningful user-readable content.
    ...
    /// Auto-generated filename patterns that rarely have recoverable identity.
    private static let autoGeneratedPattern = try? NSRegularExpression(
        pattern: #"^(Document|File|Untitled|document|file|IMG|DSC|DCIM|Screenshot|screen\s*shot)[-_\s]*\d{6,}"#,
        options: .caseInsensitive
    )

    private func prescreen(fileName: String, ext: String, sizeBytes: Int64) -> FileIntelligence.KeepOrDelete? {
        let lower = ext.lowercased()
        // Developer files — skip silently
        if Self.developerExtensions.contains(lower) { return .unsure }
        // Binary blobs — skip silently
        if Self.binaryExtensions.contains(lower) { return .unsure }
        // Auto-generated filenames with no extension hint — skip
        let bare = (fileName as NSString).deletingPathExtension
        if let re = Self.autoGeneratedPattern {
            let range = NSRange(bare.startIndex..., in: bare)
            if re.firstMatch(in: bare, range: range) != nil { return .unsure }
        }
        return nil
    }
    ...
    func analyze(filePath: String, pdfText: String?,
        ...
        // Pre-screen: avoid wasting API quota on binary/dev files
        if let prescreenDecision = prescreen(fileName: fileName, ext: ext, sizeBytes: sizeBytes) {
            // Return a minimal record — no AI call needed
            return FileIntelligence(
                filePath: filePath,
                category: ext.isEmpty ? "未知" : ext.uppercased() + " 文件",
                summary: "文件类型无法识别，已跳过 AI 分析",
                suggestedFolder: "",
                keepOrDelete: prescreenDecision,
                reason: "文件类型或命名模式不适合 AI 分析",
                confidence: 0.3,
                extractedName: nil,
                documentDate: nil,
                docType: .other,
    ...
    private func makeUserPrompt(fileName: String, ext: String,
        ...
        if let t = extractedText?.trimmingCharacters(in: .whitespacesAndNewlines), !t.isEmpty {
            ...
            let maxContent = max(0, 1000 - prompt.count)  // leave room for existingFolders
            if maxContent > 50 {
                prompt += "\ncontent:\n" + String(normalized.prefix(maxContent))
            }
        }
